...
fn main() {
    ...
    while let Some(arg) = arg_iter.next() {
        match arg.as_str() {
            "--recursive" if !collecting_directives => recursive = true,
            "--deduplicate" if !collecting_directives => deduplicate = true,
    ...
    if filelists.is_empty() {
        eprintln!(
            "Usage: {} [--recursive] [--deduplicate] [--check-exist] [--resolve-path] [--encode-with-env <NAME>] <filelist...> [-d DEFINE...]",
